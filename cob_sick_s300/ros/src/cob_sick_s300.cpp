...
class NodeClass
{
	...
	public:
		...
		NodeClass()
		{
			...
			nh.param("communication_timeout", communication_timeout, 1.5);
			...
			try
			{
				...
				else
				{
					...
					ScannerSickS300::ParamType param;
					param.dScale = 0.01;
					param.dStartAngle = 0.0/180.0*M_PI;
					param.dStopAngle = 180.0/180.0*M_PI;
			...
			topicPub_LaserScan = nh.advertise<sensor_msgs::LaserScan>("laser", 100);
		...
		void publishLaserScan(std::vector<double> vdDistM, std::vector<double> vdAngRAD, std::vector<double> vdIntensAU, unsigned int iSickTimeStamp, unsigned int iSickNow)
		{
			...
			for(int i = 0; i < (stop_scan - start_scan); i++)
			{
				if(inverted)
				{
					laserScan.ranges[i] = vdDistM[stop_scan-1-i]/100;
					laserScan.intensities[i] = vdIntensAU[stop_scan-1-i];
				}
				else
				{
					laserScan.ranges[i] = vdDistM[start_scan + i]/100;
