...
class TelegramParser {
	...
	/*	std::cout<<"HEADER"<<std::dec<<std::endl;
		...
		printf("coordination_flag : %x\n",tc.coordination_flag);
...
public:
	...
	bool parseHeader(const unsigned char *buffer, const size_t max_size, const uint8_t DEVICE_ADDR, const bool debug)
	{
		...
		if(tc2_.protocol_version==0x103)
		{
			...
			tt = *((TELEGRAM_TAIL*) (buffer+(sizeof(TELEGRAM_COMMON1)+sizeof(TELEGRAM_COMMON2)+user_data_size_+4+6)) );
			ntoh(tt);
			crc = createCRC((uint8_t*)buffer+JUNK_SIZE, full_data_size-JUNK_SIZE-sizeof(TELEGRAM_TAIL)+4+6);
		...
		//	std::cout<<"BURASI DEGİL 3\n"<<std::endl;
			...
			user_data_size_ =
				...
				(sizeof(TELEGRAM_COMMON1) + sizeof(TELEGRAM_COMMON2) - size_field_start_byte_ + crc_bytes_in_size_);
			full_data_size = sizeof(TELEGRAM_COMMON1)+sizeof(TELEGRAM_COMMON2)+user_data_size_+sizeof(TELEGRAM_TAIL);
		//	std::cout<<tt.crc<<std::endl;
		//	std::cout<<"BURASI DEGİL 5\n"<<std::endl;
			//tt = *((TELEGRAM_TAIL*)(buffer+(sizeof(TELEGRAM_COMMON1)+sizeof(TELEGRAM_COMMON2)+user_data_size_)) );
		//	std::cout<<"BURASI DEGİL 6\n"<<std::endl;
			//ntoh(tt);
			
			crc = createCRC((uint8_t*)buffer+JUNK_SIZE, full_data_size-JUNK_SIZE-sizeof(TELEGRAM_TAIL)+4+6);
		//	std::cout<<"BURASI DEGİL 7\n"<<std::endl;
			if(tt.crc!=crc)
			{
